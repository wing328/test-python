...
> AaaAuditRecordResponse get_aaa_audit_record_list()
...
with openapi_client.ApiClient(configuration) as api_client:
    ...
    try:
        # Read a 'aaa.AuditRecord' resource.
        api_response = api_instance.get_aaa_audit_record_list(filter=filter, orderby=orderby, top=top, skip=skip, select=select, expand=expand, apply=apply, count=count, inlinecount=inlinecount, at=at, tags=tags)
        pprint(api_response)
    except openapi_client.ApiException as e:
